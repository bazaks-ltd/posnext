<template>
	<div class="flex flex-col bg-gray-50 overflow-x-hidden" style="height: 100vh; max-height: 100vh;">
		...
		<template v-else>
			...
			<POSHeader
				...
				:manual-offline="offlineStore.manualOffline"
				...
				@toggle-manual-offline="offlineStore.toggleManualOfflineMode"
	...
	>
			...
			<keep-alive>
				...
				>
					<InvoiceCart
						...
						:allow-rate-change="shiftStore.allowRateChange"
		...
		<InvoiceManagement
			...
			@view-invoice="handleViewInvoice"
			@print-invoice="handlePrintInvoice"
			@share-invoice="handleShareInvoiceFromManagement"
		...
		<InvoiceShareDialog
			v-model="showInvoiceShareDialog"
			:invoice-name="uiStore.lastInvoiceName"
			:pos-profile="shiftStore.profileName"
			:customer-mobile="lastInvoiceCustomer?.mobile_no || ''"
		...
		>
			<template #body-content>
				<div class="text-center py-6">
					...
					<!-- Share Icons -->
					<div v-if="hasSharingChannels" class="mt-6 pt-6 border-t border-gray-200">
						<p class="text-sm font-medium text-gray-700 mb-3">{{ __('Share Invoice') }}</p>
						<div class="flex items-center justify-center gap-4">
							<!-- WhatsApp Share -->
							<button
								v-if="sharingOptions.whatsapp?.enabled"
								@click="handleQuickShare('whatsapp')"
								class="flex flex-col items-center gap-2 p-3 rounded-lg hover:bg-green-50 transition-colors group"
							...
							>
								...
								<span class="text-xs text-gray-600 group-hover:text-green-600">WhatsApp (API)</span>
							</button>
							
							<!-- WhatsApp Web Share -->
							<button
								v-if="sharingOptions.whatsapp_web?.enabled"
								@click="handleQuickShare('whatsapp_web')"
								class="flex flex-col items-center gap-2 p-3 rounded-lg hover:bg-green-50 transition-colors group"
								title="Share via WhatsApp Web"
							>
								<div class="w-12 h-12 rounded-full bg-green-600 flex items-center justify-center group-hover:bg-green-700 transition-colors">
									<svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
										<path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
									</svg>
								</div>
								<span class="text-xs text-gray-600 group-hover:text-green-600">WhatsApp Web</span>
							</button>
							
							<!-- SMS Share -->
							<button
								v-if="sharingOptions.sms?.enabled"
								@click="handleQuickShare('sms')"
								class="flex flex-col items-center gap-2 p-3 rounded-lg hover:bg-blue-50 transition-colors group"
								title="Share via SMS"
							>
								...
								<span class="text-xs text-gray-600 group-hover:text-blue-600">SMS</span>
							</button>
							
							<!-- Email Share -->
							<button
								v-if="sharingOptions.email?.enabled"
								@click="handleQuickShare('email')"
								class="flex flex-col items-center gap-2 p-3 rounded-lg hover:bg-purple-50 transition-colors group"
								title="Share via Email"
							>
								<div class="w-12 h-12 rounded-full bg-purple-500 flex items-center justify-center group-hover:bg-purple-600 transition-colors">
									<svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
									</svg>
								</div>
								<span class="text-xs text-gray-600 group-hover:text-purple-600">Email</span>
							</button>
						</div>
					</div>
				</div>
...
// Initialize invoice sharing
const { getSharingOptions } = useInvoiceSharing()
...
function handleInvoiceShared(data) {
	...
	showInvoiceShareDialog.value = false
	// Reset initial channel after sharing
...
async function loadSharingOptionsForInvoice(invoiceName) {
	try {
		...
		if (!shiftStore.profileName) {
			console.warn('[POSSale] No POS Profile name available')
			return
		}
...
function handleQuickShare(channel) {
	if (!uiStore.lastInvoiceName) return
	
	// Set initial channel and open share dialog
	initialShareChannel.value = channel
	uiStore.showSuccessDialog = false
	showInvoiceShareDialog.value = true
}
...
// Handle share invoice from InvoiceManagement
async function handleShareInvoiceFromManagement(invoice) {
	const invoiceName = invoice.name || invoice
	uiStore.lastInvoiceName = invoiceName
	
	// Load sharing options and customer data for this invoice
	await loadSharingOptionsForInvoice(invoiceName)
	
	// Reset initial channel (no specific channel selected)
	initialShareChannel.value = null
